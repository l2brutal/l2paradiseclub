	...
	public function run( $data, $offset )
	{
		...
		{									
			...
			if ( $extra and isset( $extra['history'] ) )
			{
				...
				foreach ( $extra['history'] as $i => $history )
				{
					if ( ( $history['s'] === 'rfnd' or $history['s'] === 'prfd' ) )
					{
						if ( $history['to'] === 'credit' )
						{
							...
							{
								$credit = $credit->add( new \IPS\Math\Number( $history['amount'] ) );
							}
							else
							{
								$credit = $credit->add( new \IPS\Math\Number( $transaction['t_amount'] ) );
							}
						}
						else
						{
							if ( isset( $history['amount'] ) )
							{
								$totalRefunded = $totalRefunded->add( new \IPS\Math\Number( $history['amount'] ) );
							}
							else
							{
								$totalRefunded = $totalRefunded->add( new \IPS\Math\Number( $transaction['t_amount'] ) );
							}
						}
					}
				}
				...
				if ( $transaction['t_status'] === \IPS\nexus\Transaction::STATUS_PART_REFUNDED and $totalRefunded->compare( new \IPS\Math\Number( $transaction['t_partial_refund'] ) ) !== 0 )
				{
					$update['t_partial_refund'] = (string) $totalRefunded;
				}
				if ( $transaction['t_status'] === \IPS\nexus\Transaction::STATUS_REFUNDED and $totalRefunded->compare( new \IPS\Math\Number( $transaction['t_amount'] ) ) !== 0 )
				{
