...
if($coinsEntregues != $coinsEntregar && $finalStatus == 3) {
	...
	$findAccID = DB::Executa("SELECT TOP 1 uid FROM user_account WHERE account = '".$account."'", "DB");
	$checkExists = DB::Executa("SELECT TOP 1 * FROM account_balance WHERE account_id = '".$findAccID[0]['uid']."'", "WORLD");
	if(count($checkExists) > 0) {
		$addBalance = DB::Executa("UPDATE account_balance SET balance = (balance+".$coinsEntregar.") WHERE account_id = '".$findAccID[0]['uid']."'", "WORLD");
	} else {
		$addBalance = DB::Executa("INSERT INTO account_balance (account_id, balance, last_update) VALUES ('".$findAccID[0]['uid']."', '".$coinsEntregar."','".date()."')", "WORLD");
	}
