			...
			<aside>
				...
				<div class='saldo'>
					<div>
						...
						if(empty($port)) {
							$saldo = DB::Executa("SELECT balance FROM account_balance WHERE account_id = '".$findAccID[0]['uid']."' LIMIT 1", "WORLD");
						} else {
							$saldo = DB::Executa("SELECT TOP 1 balance FROM account_balance WHERE account_id = '".$findAccID[0]['uid']."'", "WORLD");
						}
						if(count($saldo) == 0) { $saldo[0]['balance'] = 0; }
						?>
						<div class='total'><?php echo $LANG[10068]; ?>: <span><span><?php echo intval(trim($saldo[0]['balance']))."</span> ".$coinName_mini."" ; ?></span></div>
						<div class='bts'>
							<?php
...
<footer>
	...
	<span>&copy 2018 Lineage 2 Paradise - High-Five</span>
